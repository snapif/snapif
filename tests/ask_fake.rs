...
#[test]
fn from_env_requires_snapif_backend() {
    let _guard = ENV_LOCK.lock().expect("env lock");
    let previous = std::env::var("SNAPIF_BACKEND").ok();
    let restore = Restore(previous);
    set_backend(None);
    assert!(matches!(Client::from_env(), Err(Error::Policy(_))));

    set_backend(Some("fake"));
    let client = Client::from_env().expect("fake");
    let err = pollster::block_on(client.ask(state(), vec![department()])).expect_err("empty fake");
    assert!(matches!(
        err,
        Error::Decode(DecodeError::MissingAnswer { .. })
    ));

    set_backend(Some("typesafe"));
    assert!(matches!(Client::from_env(), Err(Error::Policy(_))));
    drop(restore);
}
...
impl Drop for Restore {
    fn drop(&mut self) {
        set_backend(self.0.as_deref());
    }
}

fn set_backend(value: Option<&str>) {
    // SAFETY: from_env_requires_snapif_backend holds ENV_LOCK, and it is the
    // only test that reads or writes SNAPIF_BACKEND.
    unsafe {
        match value {
            Some(value) => std::env::set_var("SNAPIF_BACKEND", value),
            None => std::env::remove_var("SNAPIF_BACKEND"),
        }
    }
}
